export const DRUNKARDS_PATH_PROMPT = {
...
- Four quarter-circle "pie slice" shapes, one in each corner
- Arcs are placed on opposite diagonals (COLOR1 on one diagonal, COLOR3 on the other)
- Background (COLOR2) shows through the center and between curves
...
- One of the most versatile curved-piecing patterns`,
  ...
  fabricRoleGuidance: `For Drunkard's Path specifically:
- COLOR1 (PRIMARY): Quarter-circles on one diagonal — part of the curved path
- COLOR2 (BACKGROUND): Center and negative space — defines the path shape
- COLOR3 (ACCENT): Quarter-circles on opposite diagonal — contrasting curve direction
- High contrast between curves and background emphasizes the wandering path
- Using similar values for COLOR1 and COLOR3 creates a unified meandering line`,

  cuttingInstructions: `Drunkard's Path cutting specifics:
- Cut squares for the background pieces
