...
export const STRIP_QUILT_PROMPT: PatternPrompt = {
  ...
  characteristics: `A Strip Quilt is a simple vertical strip pattern:
...
- Can be rotated 90° to create horizontal strips
- One of the fastest patterns to piece`,

  fabricRoleGuidance: `For Strip Quilt specifically:
- All fabrics are treated equally — no background/accent hierarchy
- COLOR1 through COLOR8 fill strips left to right
- Consider arranging fabrics by value (light to dark) for gradient effect
- Alternating high-contrast fabrics creates bold stripes
- Similar values create subtle, blended appearance`,

  cuttingInstructions: `Strip Quilt cutting specifics:
- Cut strips the full length of the quilt
...
- Stretching bias edges on long strips
