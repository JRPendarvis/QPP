...
export const RAIL_FENCE_PROMPT: PatternPrompt = {
  ...
  characteristics: `A Rail Fence is one of the simplest strip-pieced patterns:
...
- The magic happens in the quilt layout, not the individual block`,
  ...
  fabricRoleGuidance: `For Rail Fence specifically:
- COLOR1 (TOP): Often the lightest value — creates visual movement when tiled
- COLOR2 (MIDDLE): Medium value works well — anchors the block
- COLOR3 (BOTTOM): Often the darkest value — grounds the design
- Arrange light to dark (or dark to light) for the strongest zigzag effect when tiled`,
...
- Try different layouts: zigzag, basketweave, or random rotation`,
