...
- Each "goose" is a large triangle pointing in one direction
- Two smaller "sky" triangles flank each goose
- Geese stack vertically, all pointing the same direction (upward)
- Number of geese adjusts based on fabrics provided (1-7 geese)
- Each goose can be a different fabric; sky uses one consistent fabric
- Creates strong directional movement when tiled`,

  fabricRoleGuidance: `For Flying Geese specifically:
- COLOR1 through COLOR7: Individual goose triangles — each can showcase a different fabric
- Highest COLOR number: Sky triangles — consistent throughout, frames all geese
- Arrange goose fabrics light-to-dark for gradient effect, or alternate for contrast
- Sky fabric should contrast with geese so the points read clearly`,
...
- Build each goose unit separately (goose + two sky triangles)
- Stack and sew units together in desired order
- Press seams toward the goose (darker triangle) or open for less bulk
- Ensure all geese point the same direction before joining
...
- Geese pointing different directions (check orientation before sewing)
